...
switch($form_action){
	...
	case 'EDIT':
		...
		$sql_contact = $db->get_row("SELECT * FROM ".$table_name." WHERE ".$record_uid." ='".$reference_id."'");
		
		$post_activity_data['description'] = $sql_helper->activity_log($sql_contact,$post_data,$form_action);
		...
		header("Location: ".INDEX_PAGE.urlencode($page_name)."&mode=edit&id=".$reference_id."&success=".$result);
	...
	case 'DELETE':
		...
		if((strtoupper($_POST["Delete"]) == 'YES')){
			$sql_check = mysql_query("SELECT * FROM ".$table_name." WHERE ".$primary_id." = \"".$reference_id."\"") or die(mysql_error());
							...
							<div class="col-sm-12">
								...
								foreach($editor_fields as $editor_fields_val){
									...
									switch($editor_fields_val['type']){
										...
										break;
										case 'text':
											echo $form_object->input_text($editor_fields_val['label'],$editor_fields_val['field'],$is_editable_field,$records,$editor_fields_val['req'],$editor_fields_val['max'],$editor_fields_val['class'],$editor_fields_val['note']);
										break;
									}
								}

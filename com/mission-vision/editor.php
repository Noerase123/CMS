...
switch($form_action){
	case 'ADD':
		...
		$inser_post_data['mv_title'] = $post_data['mv_title'];
		$inser_post_data['dot'] = $post_data['dot']; 	
		$inser_post_data['fcap'] = $post_data['fcap'];
		$inser_post_data['mv_description'] = $post_data['mv_description']; 		
	...
	case 'EDIT':
			...
			//ACTIVITY LOG
			$sql_accommodations = $db->get_row("SELECT * FROM tbl_mission_vision WHERE mv_id ='".$reference_id."'");
			...
			$edit_post_data['mv_title'] = $post_data['mv_title'];
			if(!empty($post_data['dot'])){
				$edit_post_data['dot'] = $post_data['dot'];
			}
			if(!empty($post_data['fcap'])){
				$edit_post_data['fcap'] = $post_data['fcap'];
			}
	...
	case 'DELETE':
		...
		if((strtoupper($_POST["Delete"]) == 'YES')){
			...
			$sql_accommodations = $db->get_row("SELECT * FROM tbl_mission_vision WHERE mv_id ='".$reference_id."'");
			$room_title = $sql_accommodations->mv_title;

...
switch($form_action){
	...
	case 'EDIT':
		...
		$sql_banner = $db->get_row("SELECT * FROM tbl_smtp WHERE smtp_id ='".$reference_id."'");
					...
					<div class="row">
						...
						<form action="<?php //echo INDEX_PAGE . $page_option ?>" method="post" id="frm_<?php echo $page_name?>" enctype="multipart/form-data">
							...
							<div class="col-sm-12">
								...
								<?php
								$smtp = $db->get_row("SELECT * FROM ".$table_name."");
									?>
									<div class="form-group">
										<label for="smtp_port">Mail Port </label>
										<select class="form-control" name="smtp_port">
											<option value="25" <?php echo $smtp->smtp_port == 25 ? " selected = 'selected'" : "" ?>>25</option>
											<option value="2525" <?php echo $smtp->smtp_port == 2525 ? " selected = 'selected'" : "" ?>>2525</option>
											<option value="587" <?php echo $smtp->smtp_port == 587 ? " selected = 'selected'": "" ?>>587</option>
										</select>
								...
								echo $form_object->default_smtp($is_editable_field,$mode,$record->activated,$table_name,'');

...
switch($form_action){
	case 'ADD':
		...
		$inser_post_data['food_title'] = $post_data['food_title'];
		$inser_post_data['food_image'] = $post_data['food_image'];
	...
	case 'EDIT':
			...
			//ACTIVITY LOG
			$sql_accommodations = $db->get_row("SELECT * FROM tbl_food WHERE food_id ='".$reference_id."'");
			...
			$edit_post_data['food_title'] = $post_data['food_title'];
			if(!empty($post_data['food_image'])){
				$edit_post_data['food_image'] = $post_data['food_image'];
			}

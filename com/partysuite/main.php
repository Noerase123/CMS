...
if($get_module){
	...
	if(isset($_GET['mode']) || isset($_POST['form_action'])){
		...
		$chain_link = "";
		$chain_link .= "<a href=\"index.php\">Dashboard <i style = 'color:grey;' class='fa fa-chevron-right'></i></a>";
		
		for($z=0;$z<=$countlessone;$z++){
		
			$get_module_for_breadcrumbs = $db->get_row("SELECT * FROM tbl_module WHERE module_id = '".$breadcrumbs[$z]."'");
			$chain_link .= "&nbsp<li>".$get_module_for_breadcrumbs->module_name."</li>";
		...
		for($z=0;$z<=$countlessone;$z++){
			...
			if($z==$countlessone){
				...
				$chain_link .= "<li class=\"active\">".$get_module_for_breadcrumbs->module_name." </li>";

...
switch($form_action){
	case 'ADD':
		...
		$post_activity_data['description'] = "<strong>".$post_data["email_address"]."</strong>";		
	...
	case 'EDIT':
		...
		$post_activity_data = array();
		$sql_user = $db->get_row("SELECT * FROM tbl_user WHERE user_id ='".$reference_id."'");
		
		$post_activity_data['description'] = $sql_helper->activity_log($sql_user,$post_data,$form_action);	
	...
	case 'DELETE':
		...
		if((strtoupper($_POST["Delete"]) == 'YES')){
			...
			$sql_user_manager = $db->get_row("SELECT * FROM tbl_user WHERE user_id ='".$reference_id."'");
			$user_email = $sql_user_manager->email_address;
			...
			$post_activity_data['description'] = "<strong>".$user_email."</strong>";		
			$post_activity_data['date_updated'] = $post_data['date_updated'];
			$post_activity_data['ip_address'] = $_SERVER['REMOTE_ADDR'];			
			$result = $sql_helper->insert_all('tbl_activity_log',$post_activity_data);
		...
		<div class="col-lg-12">
			<div class="panel panel-primary">
				<div class="panel-heading">
					...
					?>
						<div class="standard-form-instruction">
							<strong>Note:</strong> <?php echo $req_fld ?> denotes required field.
						</div>
					<?php 
						}else{
					?>
				...
				<div class="panel-body">
					<div class="row">
						...
						?>
						<Div class="col-sm-12">
							<form action="<?php echo INDEX_PAGE . $page_option ?>" id="frm_<?php echo $page_name?>" method="post" name="frm_<?php echo $page_name; ?>">
							<input type="hidden" name="form_action" value="<?php echo strtoupper($mode)?>">
							<input type="hidden" name="mode" value="<?php echo $mode?>">	
							<?php
								$password = $form_object->input_password($record->password,$record->varkey);
								...
								foreach($editor_fields as $editor_fields_val){
									...
									switch($editor_fields_val['type']){
										...
										break;
										case 'text':
											echo $form_object->input_text($editor_fields_val['label'],$editor_fields_val['field'],$is_editable_field,$records,$editor_fields_val['req'],$editor_fields_val['max'],$editor_fields_val['class'],$editor_fields_val['note'],$password);
										break;
										case 'select':
						...
						<form action="<?php //echo INDEX_PAGE . $page_option ?>" method="post" id="frm_<?php echo $page_name?>" enctype="multipart/form-data">
							...
							<div class="col-sm-12">
								...
								foreach($editor_fields as $editor_fields_val){
									...
									if($editor_fields_val['field'] == 'user_role_id'){
										$result_list = $db->get_results("SELECT * FROM tbl_user_role WHERE user_role_id =". $record->$editor_fields_val['field']."");
										foreach($result_list as $result_list_val_2){
											$user_role_title = $result_list_val_2->user_role_title;
										}
										$records = $user_role_title;
									}
									else{
										$records = $record->$editor_fields_val['field'];
									}
								...
								echo $form_object->enabled($is_editable_field,$mode,$record->activated,'');
